                        ...
                        <div class="schedule-filter">
                            <select name="teacher" onchange="this.form.submit()">
                                ...
                                @foreach ($data['teachers'] as $teacher)
                                    <option value="{{ is_array($teacher) ? $teacher['fio'] : $teacher->fio }}" {{ $data['selectedTeacher'] === (is_array($teacher) ? $teacher['fio'] : $teacher->fio) ? 'selected' : '' }}>
                                        {{ is_array($teacher) ? $teacher['fio'] : $teacher->fio }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        <div class="schedule-filter">
                            <select name="subject" onchange="this.form.submit()">
                                ...
                                @foreach ($data['subjects'] as $subject)
                                    <option value="{{ $subject }}" {{ $data['selectedSubject'] === $subject ? 'selected' : '' }}>{{ $subject }}</option>
                                @endforeach
                            </select>
                        </div>
                    </form>
                ...
                <div class="grid-container-calendar">
                    ...
                    @for ($hour = 8; $hour <= 22; $hour++)
                        @for ($day = 1; $day <= 7; $day++)
                            ...
                            @if ($lesson)
                                @php
                                    $lessonId = md5($lesson['subject'].$lesson['teacher'].$lesson['start_time'].$lesson['end_time'].$lesson['date_']);
                                ...
                                @if ($isFirstHour && !in_array($lessonId, $rendered))
                                    <div class="cell has-lesson"
                                        ...
                                        <div style="padding:6px 8px;">
                                            ...
                                            <div style="font-size:13px;color:#374151;">{{ $lesson['teacher'] }}</div>
